...
Behavior:
  1) Builds runtime bundle via scripts/ops/create_runtime_bundle.sh (unless --no-build).
...
if [[ "$NO_BUILD" -eq 0 ]]; then
  BUILD_CMD=("$REPO_ROOT/scripts/ops/create_runtime_bundle.sh" --output-dir "$OUTPUT_DIR" --bundle-name "$BUNDLE_NAME")
...
ssh "${SSH_OPTS[@]}" "$SSH_TARGET" "set -euo pipefail; cd '$REMOTE_DIR'; bash ./scripts/ops/verify_release_manifest.sh --path '$REMOTE_DIR/RELEASE_MANIFEST.txt'"
