...
- Multi-agent **orchestration** uses repo-root **`governance/`** (protocols: registry, lifecycle, deploy, branching) plus **`plans/`** (tactical markdown under `plans/cursor/`); the VPS does not mirror those trees. SSH there for runtime truth, read `governance/` + `plans/` here for coordinated work definition. If another local agent tool mirrors or overlaps this tree, add its paths to **.cursorignore** so Cursor does not double-index conflicting copies.
