...
- Production hostnames web `thehivemanager.com` and API `api.thehivemanager.com` terminate on the VPS loopback (`127.0.0.1:8443`) behind Cloudflare tunnel; public uptime is the Cloudflare edge on 443. GitHub Actions Health Check curls those public URLs (not origin `:8443` `--resolve`). A Cloudflare `403` on GitHub-hosted runner IPs is WAF/bot reputation, not origin-down. `vps_state.sh` / `sprint_verify.sh` still read `VPS_ORIGIN_IP` from repo-root `.env` (allowlisted, not shell-sourced).
