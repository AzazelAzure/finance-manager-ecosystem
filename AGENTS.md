...
- **PWA/offline product bar:** Core ledger surfaces (transactions, calendar, flows, source balances) should stay coherent when offline for extended periods (days to weeks), with local-first behavior and API sync when back online; offline FX scope can be limited to currencies already present on the user’s sources when that keeps the offline window practical.
