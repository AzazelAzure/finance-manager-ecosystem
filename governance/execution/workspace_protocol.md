...
- **Local MCP server — implemented (Cursor, PR #88 pending merge).** Delivered with `tp-scripts-organization/` taxonomy reorg on the same branch: `scripts/mcp/` FastMCP stdio server (`hfm-scripts`) wraps Tier 1–2 scripts and workspace queue tools; project config `.cursor/mcp.json`. Per D8 (2026-07-01), this was reordered *ahead* of the full F-006/F-009 pilot — build the MCP wrap first, then run the live pilot through typed tools. Unblocks the real pilot once PR #88 merges and agents reload MCP in the IDE. See `scripts/mcp/README.md`.
