...
plan_root: plans/S1/S1.B/pwa-implementation-branch/
...
- **Offline — PR #52 / #53 on active green (deployed); offline ledger still broken:** [`finance-manager-web` PR #52](https://github.com/AzazelAzure/finance-manager-web/pull/52) plus build hotfix [**PR #53**](https://github.com/AzazelAzure/finance-manager-web/pull/53) shipped **form-parity and build-fix** code (Data Hub invalidations, lookup **`create*`** / 202, QuickActions category queue, dashboard snapshot **`forceNetwork`** guard when **`preferOfflineCaches()`**, profile Dexie merge on enqueue, FIFO rename overlay on merged tx rows, **`getTransaction`** local-first under **`preferPwaLocalFirstReads()`**). **These changes do not mean offline transaction population is fixed** — HitM reports **transactions still do not populate when offline**; **BP_OFFLINE_READ** stays **not PASS**. See [`runtime_handoff.md`](runtime_handoff.md) for **offloading offline→online**, **inactive-only** test policy, **auto-merge disabled**, and the **detailed change log**. Further fixes: **inactive color** only until verified; **D4-exec** remains manual on `:8443`.
