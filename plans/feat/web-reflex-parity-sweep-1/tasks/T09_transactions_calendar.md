...
- [x] Range controls: month-picker + arrow nav (prev/next month), with
      manual `start_date`/`end_date` advanced fields.
- [x] Toggles: display currency mode, heat metric mode.
...
### Tables
- [x] **Due events** for the range: name, due date, amount, status.
- [x] **Day drill** when a day is clicked: list of transactions for that day.

### Progress note (2026-04-29)
- Initial calendar page scaffold is live with backend data wiring, filter
  controls, day selection, due events table, and day-drill table.
- Remaining items: dedicated monthly heat-grid component, explicit monthly bar
  chart module, richer keyboard/a11y behavior for day navigation, and viewport
  polish targets.
