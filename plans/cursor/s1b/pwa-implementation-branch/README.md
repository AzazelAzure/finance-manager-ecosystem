...
- **Offline — PR-3 shipped (web, pending merge/deploy):** [`finance-manager-web` PR #52](https://github.com/AzazelAzure/finance-manager-web/pull/52) (`feature/pwa-offline-e2e-pr3`) closes the **form-parity** batch: Data Hub invalidates `sources`/`categories`/`tags`/`snapshot`/`transactions`; lookup **`create*`** returns **`OfflineQueuedResult`** on synthetic 202; **QuickActions** refreshes category reads after offline-queued category create; **Dashboard** forced snapshot avoids **`forceNetwork`** when **`preferOfflineCaches()`**; **`PATCH /finance/appprofile/`** updates Dexie **`appprofile:root`**; **FIFO rename overlay** on merged **transaction** rows for category/source/tag PATCH; **`getTransaction`** uses **`findTransactionRecordById`** under **`preferPwaLocalFirstReads()`**. After merge: VPS pull/rebuild + **D4-exec** window per [`runtime_handoff.md`](runtime_handoff.md).
