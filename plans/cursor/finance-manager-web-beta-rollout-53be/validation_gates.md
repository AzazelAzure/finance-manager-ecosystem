...
- **2026-04-30:** **OPEN (ready to test)** — Prod API is available for Lane B. Run smoke from **https://jsdevtesting…** / **jsdevprodtest…** / or local 5173 with `VITE_API_BASE_URL=https://api.thehivemanager.com`. **Lane A** in web README still valid to isolate the UI from prod. Log status + CORS if failure.
