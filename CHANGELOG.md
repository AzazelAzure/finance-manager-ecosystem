...
- **Sprint pipeline gap (documented):** [`governance/sprint_queue_message_spec_v1.md`](governance/sprint_queue_message_spec_v1.md) **Pipeline continuity** + [`governance/cursor_pa_slack_visibility.md`](governance/cursor_pa_slack_visibility.md) — `#sprint-queue` intake alone does not post `#review-queue` or next slice; Phase 1 handoff is explicit per relay contract; automation requires PA extension or manual orchestration.
- **Sprint Slack bridge:** [`scripts/sprint_slack_pipeline_bridge.py`](scripts/sprint_slack_pipeline_bridge.py) — daemon-style poller for `SPRINT_PIPELINE_JSON` (`READY_FOR_REVIEW` / `REVIEW_VERDICT`), optional V0 auto-pass, next-slice file under plan `evidence/pipeline_queue/`, HitM gate when `requires_hitm`. Spec: [`governance/sprint_queue_message_spec_v1.md`](governance/sprint_queue_message_spec_v1.md) §Machine-readable pipeline.
