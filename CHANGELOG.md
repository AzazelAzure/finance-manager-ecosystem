...
- **Sprint Slack bridge:** [`scripts/sprint_slack_pipeline_bridge.py`](scripts/sprint_slack_pipeline_bridge.py) — Web API poller for `SPRINT_PIPELINE_JSON` (`READY_FOR_REVIEW` / `REVIEW_VERDICT`), optional **`SPRINT_PIPELINE_LOCAL_INBOX`** JSONL on the same host (no sprint-thread READY required), [`scripts/sprint_pipeline_emit_ready.py`](scripts/sprint_pipeline_emit_ready.py) helper, default **`SPRINT_BRIDGE_AUTO_PASS_IF_NON_HITM`** so slices with `requires_hitm: false` auto-chain without a human `#review-queue` PASS (disable per host when you want manual review), V0-only fallback via **`SPRINT_BRIDGE_AUTO_PASS_V0`**, next-slice file under `SPRINT_BRIDGE_NEXT_MESSAGE_BASEDIR`, HitM gate when `requires_hitm`. Spec: [`governance/sprint_queue_message_spec_v1.md`](governance/sprint_queue_message_spec_v1.md) §Machine-readable pipeline.
