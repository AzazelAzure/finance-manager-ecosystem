...
> **DRAFT STATUS:** This document is a pre-publication draft prepared for attorney review. It is NOT legal advice and has NOT been reviewed by a qualified Philippine attorney. Sections marked `[ATTORNEY REVIEW]` must be reviewed before publication. Sections marked `[PENDING T05]` or `[PENDING T06]` will require revision once the corresponding security changes are deployed. Sections marked `[PLACEHOLDER]` will be completed when the corresponding feature is implemented.
...
> **`[ATTORNEY REVIEW]`** Whether F-003's predictive output constitutes "automated decision-making or profiling" triggering NPC registration under RA 10173 (regardless of user count) must be assessed by a qualified Philippine attorney before F-003 is publicly launched.
...
> **`[ATTORNEY REVIEW]`** The cross-border transfer contractual clause with Namecheap/Cloudflare under NPC Advisory 2024-01 must be drafted and executed by a qualified Philippine attorney before publication.
...
> **`[ATTORNEY REVIEW]`** Self-declaration without independent verification — attorney to confirm this approach is adequate under PH-DPA requirements for a financial application. See also N8 in open items.
...
> **`[ATTORNEY REVIEW]`** Whether F-003 Predictive Budgeting constitutes "automated decision-making or profiling" requiring NPC registration regardless of user count — attorney assessment required before F-003 launches publicly.
