...
> **DRAFT STATUS:** This document is a pre-publication draft prepared for attorney review. It is NOT legal advice and has NOT been reviewed by a qualified Philippine attorney. Sections marked `[ATTORNEY REVIEW]` must be reviewed before publication. Sections marked `[PENDING T05]` or `[PENDING T06]` require revision once those security changes are deployed. Sections marked `[PLACEHOLDER]` will be completed when the corresponding feature is implemented.
...
> **`[ATTORNEY REVIEW]`** Confirm that classification of Dexie local financial data storage as "functional" (necessary to deliver the requested service) is defensible under the Philippine Data Privacy Act and relevant NPC guidance.
